...
static GLuint InitShader(const char* code, GLint type)
{
    ...
    if(sh == 0){
        ...
        XLOGE("glCreateShader failed! %d",num);
        return 0;
    }
    //加载shader
    ...
    if(status == GL_FALSE)
    {
        ...
        glGetShaderInfoLog(sh, infoLogLength, NULL, strInfoLog);

        XLOGE("Compilation error in shader : %s\n", strInfoLog);
        delete[] strInfoLog;
        return 0;
    }
...
bool XShader::Init(XShaderType type) {
    ...
    if(program == 0)
    {
        mux.unlock();
        GLenum num = glGetError();
        XLOGE("glCreateProgram failed! %d", num);
        return false;
    }
